...
def get_debiasing_projection(classifier_class, cls_params: Dict, num_classifiers: int, input_dim: int,
    ...
    for i in pbar:
        ...
        dropout_mask = (
            np.random.rand(*X_train.shape) < (1 - dropout_rate)
        ).astype(float) * dropout_scale
...
if __name__ == '__main__':
    ...
    Y = np.array([1 if sum(x) > 0 else 0 for x in X])
    ...
    P, rowspace_projections, Ws = get_debiasing_projection(classifier_class, {
    }, num_classifiers, input_dim, is_autoregressive, min_accuracy, X, Y, X, Y, by_class=False)
